...
# Run 'npx hardhat node' in a separate terminal first
w3 = Web3(Web3.HTTPProvider('http://127.0.0.1:8545'))

# 2. Project Paths (Handles folder separation)
BASE_DIR = Path(__file__).resolve().parent.parent.parent
BLOCKCHAIN_DIR = BASE_DIR / 'blockchain' / 'student-reward-system'

# 3. Contract Addresses 
# REPLACE THESE with the ones from your 'deployed_addresses.json'
EDU_TOKEN_ADDR = "0x5FbDB2315678afecb367f032d93F642f64180aa3"
MARKET_ADDR = "0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512"

# 4. Admin Credentials (Account #0 from 'npx hardhat node')
...
def award_edutokens(student_wallet_address, amount):
    """
    Django calls this when a quiz is finished.
    Admin pays the gas (sponsored transaction).
    """
    if not student_wallet_address:
        return None, "Student has no wallet linked."
    ...
    try:
        tx = contract.functions.awardStudent(student_wallet_address, amount).build_transaction({
            ...
            'gas': 150000,
            'gasPrice': w3.eth.gas_price # Matches your 0-gas Hardhat config
        })
...
def execute_marketplace_purchase(student_wallet_address, cost):
    """
    The Admin (Backend) calls the Marketplace to process the student's purchase.
    The Admin pays the GAS, but tokens are pulled from the STUDENT wallet.
    """
    if not student_wallet_address:
        return None, "No wallet linked to student"

    market = get_market_contract()
    try:
        # We call 'processPurchase' instead of 'buyItem'
        tx = market.functions.processPurchase(student_wallet_address, cost).build_transaction({
            'from': ADMIN_ADDR,
            'nonce': w3.eth.get_transaction_count(ADMIN_ADDR),
            'gas': 200000,
            'gasPrice': w3.eth.gas_price # Sponsored/Free on Localhost
        })
...
def get_live_balance(student_wallet_address):
    ...
    if not student_wallet_address:
        return 0
    contract = get_token_contract()
    balance_wei = contract.functions.balanceOf(student_wallet_address).call()
    return balance_wei / 10**18 # Convert from Wei to EDU